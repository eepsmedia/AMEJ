...
In the article in the AMEJ,
we gave an example looking at pulse data from students and teachers. 
We used a way cool technique called a randomization test in which we
scrambled data values to make the null hypothesis true.

We'll let you work with that data in a bit, and reproduce what we did in the journal.
But first, let's walk a slightly different path with similar data.

We'll begin just doing more exploratory data analysis (EDA).
